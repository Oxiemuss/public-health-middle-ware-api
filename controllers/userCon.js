...
exports.addUser = async (req, res) => {
  ...
  if (!user_name || !pass_word || !hcode) {
    return res
      .status(400)
      .json({ error: "กรุณาระบุ user_name, pass_word และ hcode ให้ครบถ้วน" });
  }
  ...
  if (error) {
    if (error.code === "23505")
      return res.status(403).json({ error: "Username นี้ถูกใช้งานแล้ว" });
    if (error.code === "23503")
      return res.status(400).json({ error: "รหัส hcode หน่วยงานไม่ถูกต้อง" });
    return res.status(400).json({ error: error.message });
  }
  res
    .status(201)
    .json({ message: "สร้างบัญชีผู้ใช้งานเรียบร้อย", data: data[0] });
};
...
exports.getAllUsers = async (req, res) => {
  ...
  const result = data.map((user) => ({
    ...
    hospital_name: user.health_centers
...
exports.delUser = async (req, res) => {
  try {
    ...
    if (!user_name) {
      return res
        .status(400)
        .json({ error: "กรุณาระบุ user_name ที่ต้องการลบ" });
    }
    ...
    res.json({
      message: `ลบผู้ใช้ ${user_name} เรียบร้อยแล้ว`,
      deletedData: data,
    });
  } catch (err) {
...
exports.updateUser = async (req, res) => {
  try {
    ...
    if (!user_name) {
      return res
        .status(400)
        .json({ error: "กรุณาระบุ user_name ที่ต้องการแก้ไข" });
    }
    ...
    if (error) {
      ...
      if (error.code === "23503") {
        return res
          .status(400)
          .json({ error: "รหัส hcode ที่ระบุไม่มีอยู่ในระบบ" });
      }
      return res.status(400).json({ error: error.message });
    }
...
exports.getUserByUsername = async (req, res) => {
  try {
    ...
    const result = {
      ...data,
      hospital_name: data.health_centers
        ? data.health_centers.h_name
        : "ไม่ระบุสังกัด",
    };
